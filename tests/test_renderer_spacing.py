...
class ContractionSpacingTests(unittest.TestCase):
    ...
    def test_contractions_do_not_use_phase_dependent_compact_cells(self) -> None:
        """Pack common contractions identically regardless of apostrophe phase."""
        expected = {
            "You're": (("literal", "Yo"), ("literal", "u'"), ("literal", "re")),
            "didn't": (("literal", "di"), ("literal", "dn"), ("literal", "'t")),
        ...
        for text, units in expected.items():
            with self.subTest(text=text):
                ...
                self.assertEqual(tuple(cell for cell in packed[1] if cell[1] != "  "), units)
                self.assertFalse(any(style == "compact" for style, _unit in packed[1]))

    def test_apostrophe_mark_is_centered_inside_its_six_pixel_slot(self) -> None:
        """Keep the narrow mark away from either edge without changing advance."""
        leading = font_render._bytes_matrix(font_render.render_literal_cell("'t"))
        trailing = font_render._bytes_matrix(font_render.render_literal_cell("u'"))
    ...
    def test_apostrophe_contraction_is_not_an_allowed_compact_cluster(self) -> None:
