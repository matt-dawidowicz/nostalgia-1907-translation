...
reports every translated record, every structurally proven SCN occurrence, its
renderer/box class, row-by-row cell use, native opening gutter, and any
...
The existing compiler remains authoritative for emitted MES bytes. SCN display
occurrences come from ``scn_layout.display_occurrences`` so the compiler/editor
and this human-reviewable audit share one command-shape parser instead of
maintaining competing opcode scanners.
"""
...
from .scn_layout import display_occurrences
