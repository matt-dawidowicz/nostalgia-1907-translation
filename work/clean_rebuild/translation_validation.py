...
def validate(comparison_json: Path, *, require_comparison: bool = True) -> dict[str, object]:
    ...
    failures: list[str] = []
    if normalize_english_for_semantic_comparison("same\n  words") != normalize_english_for_semantic_comparison("same words"):
    ...
    if exact["conflicting_duplicate_group_count"]:
        failures.append("identical Japanese source fingerprints have conflicting English")
    ...
    for term in glossary["fixed_source_terms"]:
        ...
        if representative is None or representative["normalized_bitmap_sha256"] != term["normalized_bitmap_sha256"]:
        ...
        for record in by_fingerprint.get(term["normalized_bitmap_sha256"], []):
            if record["policy"] == "translate" and record["english"] != term["canonical_english"]:
                failures.append(
    ...
    for term in glossary["controlled_phrases"]:
        for record_id in term.get("record_ids", []):
            ...
            if record is None:
                failures.append(f"{term['id']}: controlled record is missing: {record_id}")
    ...
    for expected in glossary.get("source_authoritative_records", []):
        ...
        expected_english = normalize_english_for_semantic_comparison(expected["expected_english"])
    ...
    for exception in glossary.get("reviewed_contextual_exceptions", []):
        ...
        if record["normalized_bitmap_sha256"] != exception["normalized_bitmap_sha256"]:
            failures.append(f"{record_id}: contextual-exception Japanese fingerprint changed")
    ...
    for rule in repairs["bomb_term_replacements"]:
        ...
        if any(pattern.sub(rule["replacement"], sample) != sample for sample in ("cord", "cords")):
    ...
    for table_name in ("reviewed_visible_translation_exemptions", "reviewed_control_records"):
    ...
    for chapter_item in index["chapters"]:
        ...
        if not isinstance(records, list) or len(records) != source.get("record_count"):
            ...
            continue
        if any(record.get("index") != position for position, record in enumerate(records)):
        ...
        if translated != chapter_item.get("translated_records") or preserved != chapter_item.get("preserved_records"):
    ...
    part3b_preserved = {record["index"] for record in part3b["records"] if record["policy"] == "preserve"}
    ...
    if comparison_json.exists():
        ...
        if generated_ids != expected_ids:
            failures.append("generated comparison record IDs/order differ from canonical source")
    ...
    return {
        ...
        "formatting_only_duplicate_variants": exact["formatting_only_duplicate_group_count"],
        ...
        "reviewed_contextual_exceptions": len(glossary.get("reviewed_contextual_exceptions", [])),
...
def main() -> None:
    ...
    payload = validate(args.comparison_json, require_comparison=not args.allow_missing_comparison)
