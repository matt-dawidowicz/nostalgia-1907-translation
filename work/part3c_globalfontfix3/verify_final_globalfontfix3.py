#!/usr/bin/env python3
"""Verify the font-compatible corrected PART3C BIN/CUE delivery."""
...
LZ_PATH = DELIVERY / "PART3C_globalfontfix3.LZ"
ISO_PATH = DELIVERY / "Nostalgia1907_Act3C_000_223_globalfontfix3.iso"
TRACK1_PATH = DELIVERY / "Nostalgia1907_Act3C_000_223_globalfontfix3_Track1.bin"
TRACK2_PATH = DELIVERY / "Nostalgia1907_Act3C_000_223_globalfontfix3_Track2.bin"
CUE_PATH = DELIVERY / "Nostalgia1907_Act3C_000_223_globalfontfix3.cue"
...
def normalized_prose(text: str) -> str:
    """Remove only row-end structural padding."""
...
def controls(record: bytes) -> list[bytes]:
    """Return line/page controls plus the terminator."""
    return [token for token in tokens(record) if token[0] in (0xEE, 0xEF)] + [
        b"\x00"
    ]


def glyph_bitmap(token: bytes, font: bytes, tail: bytes) -> bytes | None:
    """Resolve one glyph token to the bitmap the game displays."""
    if len(token) == 2:
...
def glyph_bitmaps(record: bytes, font: bytes, tail: bytes) -> list[bytes]:
    """Return the ordered rendered glyph bitmaps in a record."""
    result: list[bytes] = []
    for token in tokens(record):
        bitmap = glyph_bitmap(token, font, tail)
...
def trailing_blank_cells(record: bytes, font: bytes, tail: bytes) -> int:
    """Count final glyph cells with no ink."""
    count = 0
    for token in reversed(tokens(record)):
        if glyph_bitmap(token, font, tail) != bytes(GLYPH_BYTES):
            break
        count += 1
    return count
...
def layouts(raw: dict[str, object]) -> dict[int, tuple[int, int]]:
    """Convert JSON layout values to integer tuples."""
    return {
...
def member_hashes(root: Path) -> dict[str, str]:
    """Hash actual unpacked LZ members."""
...
def verify_raw_payload(raw_path: Path, iso_path: Path) -> dict[str, object]:
    """Verify every raw Track 1 user-data sector against the ISO."""
    raw_sectors = raw_path.stat().st_size // RAW_SECTOR_SIZE
    iso_sectors = iso_path.stat().st_size // DATA_SECTOR_SIZE
    if (
        raw_path.stat().st_size % RAW_SECTOR_SIZE
        or iso_path.stat().st_size % DATA_SECTOR_SIZE
        or raw_sectors != iso_sectors
    ):
        raise ValueError("raw Track 1/ISO sector geometry differs")
    ...
    with raw_path.open("rb") as raw, iso_path.open("rb") as iso:
        for sector in range(iso_sectors):
            ...
            if user_data != iso_sector:
                raise ValueError(f"raw Track 1 differs from ISO at sector {sector}")
            payload_hash.update(user_data)
    return {
        "sectors": iso_sectors,
        "all_user_data_sectors_match_iso": True,
        "user_data_sha256": payload_hash.hexdigest().upper(),
    }


def main() -> None:
    """Run the independent delivery audit."""
    if REPORT.exists():
    ...
    global_font = FONT_PATH.read_bytes()
    v4_font = (V4 / "FIX_CODE.FNT").read_bytes()
    if global_font != (V3 / "FIX_CODE.FNT").read_bytes():
        raise ValueError("delivered global font differs from visualfix3")
    if info.split_offset != v4_info.split_offset or info.split_offset > TEXT_BOUNDARY_LIMIT:
        raise ValueError("remapped MES text boundary changed")
    if data[: 4 + POINTER_COUNT * 2] != v4_data[: 4 + POINTER_COUNT * 2]:
        raise ValueError("remapped MES pointer table changed")
    if tail != v4_tail:
        raise ValueError("remapped MES dynamic tail changed")
    if records[158] != v3_records[158]:
        raise ValueError("untranslated record 158 changed")

    checked_glyphs = 0
    for index in sorted(expected_translated):
        if len(records[index]) != len(v4_records[index]):
            raise ValueError(f"record {index} size changed during remap")
        if controls(records[index]) != controls(v4_records[index]):
            raise ValueError(f"record {index} controls changed during remap")
        final_bitmaps = glyph_bitmaps(records[index], global_font, tail)
        source_bitmaps = glyph_bitmaps(v4_records[index], v4_font, v4_tail)
        if final_bitmaps != source_bitmaps:
            raise ValueError(f"record {index} rendered bitmaps changed during remap")
        units = entries[index]["units"]
        if len(final_bitmaps) != len(units):
            raise ValueError(f"record {index} manifest glyph count changed")
        for position, (bitmap, unit) in enumerate(zip(final_bitmaps, units)):
            ...
            if bitmap != expected:
                raise ValueError(
                    f"record {index} glyph {position} differs from manifest"
                )
            checked_glyphs += 1
    if checked_glyphs != 6268:
        raise ValueError(f"manifest glyph total changed: {checked_glyphs}")
    ...
    if forced != FORCED_PADDING:
        raise ValueError("forced-padding record set changed")
    if any(not entries[index]["pad_final_row"] for index in protected):
        raise ValueError("a protected record lacks final-row padding")
    if protected & set(config.get("no_pad_final_row_segments") or ()):
        raise ValueError("a protected record appears in the no-pad set")
    actual_blanks = {
        index: trailing_blank_cells(records[index], global_font, tail)
    ...
    if runtime["segment_count"] != 208 or runtime["row_count"] != 617:
        raise ValueError("runtime row totals changed")
    ...
    if len(floating) != 38:
        raise ValueError("floating-window count changed")
    ...
    if (ISO_EXTRACT / "FIX_CODE.FNT").read_bytes() != global_font:
        raise ValueError("ISO-extracted global font changed")
    if (UNPACKED / "001_PART3C.MES.unpacked").read_bytes() != data:
        raise ValueError("delivered LZ does not contain delivered MES")
    if b"FIX_CODE.FNT" not in (ISO_EXTRACT / "MAIN.BIN").read_bytes():
        raise ValueError("MAIN.BIN no longer references the global fixed font")
    ...
    if len(regression_chunks) != 564 or len(regression_mes) != 21:
        raise ValueError("full regression artifact counts changed")
    ...
    report = {
        ...
        "part3c_contract": {
            ...
            "control_tokens_identical_to_padding_corrected_source": True,
            "record_sizes_identical_to_padding_corrected_source": True,
            "dynamic_tail_identical_to_padding_corrected_source": True,
            "rendered_bitmaps_identical_to_padding_corrected_source": True,
            "untranslated_record_158_byte_identical": True,
            "checked_manifest_glyphs": checked_glyphs,
        ...
        "archive_contract": {
            ...
            "iso_embedded_global_font_exact": True,
        },
        "full_regression": {
