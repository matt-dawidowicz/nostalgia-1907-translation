#!/usr/bin/env python3
"""Build the complete PART3C translation test BIN/CUE in retail LZ slots."""
...
SOURCE_REGRESSION = WORKSPACE / "outputs" / "PART3C_rowparityfix6_fresh" / "regression_full"
DELIVERY = WORKSPACE / "outputs" / "PART3C_transitionfix10_full_fresh"
...
OUTPUT_LZ = DELIVERY / "PART3C_transitionfix10.LZ"
OUTPUT_ISO = DELIVERY / "Nostalgia1907_Act3C_transitionfix10_full.iso"
OUTPUT_TRACK1 = DELIVERY / "Nostalgia1907_Act3C_transitionfix10_full_Track1.bin"
OUTPUT_TRACK2 = DELIVERY / "Nostalgia1907_Act3C_transitionfix10_full_Track2.bin"
OUTPUT_CUE = DELIVERY / "Nostalgia1907_Act3C_transitionfix10_full.cue"
...
def main() -> None:
    """Pack the full guarded MES and produce fresh raw Sega CD tracks."""
    if DELIVERY.exists():
        raise FileExistsError(f"refusing to reuse {DELIVERY}")
    mes_report = json.loads(
        (HERE / "transitionfix10_mes_report.json").read_text(encoding="utf-8")
    )
    if mes_report.get("status") != "PASS" or not mes_report.get("release_candidate"):
        raise ValueError("full transition MES report is not clean")

    DELIVERY.mkdir(parents=True)
    shutil.copyfile(HERE / "PART3C_transitionfix10.MES", OUTPUT_MES)
    shutil.copyfile(HERE / "PART3C_transitionfix10.SCN", OUTPUT_SCN)
    shutil.copyfile(SOURCE / "FIX_CODE.FNT", OUTPUT_FONT)
    shutil.copytree(SOURCE_REGRESSION, REGRESSION)
    shutil.copyfile(OUTPUT_MES, REGRESSION / "PART3C" / "001_PART3C.MES.unpacked")
    shutil.copyfile(OUTPUT_SCN, REGRESSION / "PART3C" / "000_PART3C.SCN.unpacked")
    ...
    if len(original_entries) != 52 or len(output_entries) != len(original_entries):
        raise ValueError("PART3C LZ member count changed")
    if [item.offset for item in original_entries] != [item.offset for item in output_entries]:
        raise ValueError("full transition LZ changed retail member offsets")
    if OUTPUT_LZ.stat().st_size != ORIGINAL_LZ.stat().st_size:
        raise ValueError("full transition LZ changed retail archive length")

    unpack_lz(OUTPUT_LZ, UNPACKED)
    if (UNPACKED / "001_PART3C.MES.unpacked").read_bytes() != OUTPUT_MES.read_bytes():
        raise ValueError("archive MES round-trip mismatch")
    if (UNPACKED / "000_PART3C.SCN.unpacked").read_bytes() != OUTPUT_SCN.read_bytes():
        raise ValueError("archive SCN round-trip mismatch")
    ...
    if set(original_members) != set(output_members) or changed_from_retail != [
        ...
        raise ValueError(f"unexpected archive changes from retail: {changed_from_retail}")
    for name in (
        ...
        if (UNPACKED / name).read_bytes() != (ORIGINAL_ARCHIVE / name).read_bytes():
            raise ValueError(f"protected archive member changed: {name}")
    ...
    if iso_payload(OUTPUT_ISO, "PART3C.LZ") != OUTPUT_LZ.read_bytes():
        raise ValueError("ISO-embedded PART3C.LZ differs from output archive")
    ...
    if (
        ...
        raise ValueError("full transition disc failed boot/CUE validation")
    ...
    report = {
        ...
        "purpose": "complete 224-record PART3C translation with proven retail transition geometry",
        "mes_report": mes_report,
        "mes": facts(OUTPUT_MES),
        "scn": facts(OUTPUT_SCN),
        "font": facts(OUTPUT_FONT),
        "lz": facts(OUTPUT_LZ),
        "archive_members": len(output_entries),
        ...
        "protected_screen_and_background_members_byte_identical_to_retail": True,
